...
The key "description" is a narration, acting like dialogue in the game. ##If key "description" exists, What you have to do is to directly output the value of "description" into "text" remaining unchanged.
The key "next" indicates the next step of the current step. When the value of "next" contains only "step", Output the value of "next" into "next_step" in your answer,Output the current scene into "next_scene" in your answer, However, when the value of "next" contains both "scene" and "step", it means the scene is gonna change.Output the step in "next" into "next_step" and scene in "next" into "next_scene".
The output of "text" at this time is conceived and generated by GPT itself. In this time, the value of the key "GPT_chararter" represents the role that GPT needs to play in this step. Please play the role you according to the profile, personality and lines in the dictionaries "profile_鈴木あい" and "profile_佐々木美咲". Be as vivid as possible.
If key "button" exists, Output the value of key "button" into "button" in your answer(True). Otherwise output False
##If key "answer" exists, Output the value of key "answer" into "answer" in your answer(True). Otherwise output False.
If key "options" exists, Output the value of key "options" into "options" in your answer. Otherwise output a empty list. ##Do not output empty values such as None or Null.
The key "image" indicates whether an image needs to be output in the current step. When "image" is True, output the value(string) of "image_class" into "image_class" in your JSON file.When "image" does not exist, output the "image_class" in None.
Output the value of key "timecount" into "timecount" in your answer(True or False).
...
Remember, if the user mentioned his or her name, any mention of the "username" in description will automatically be replaced by the user's entered name.
...
storyline = {
    ...
    "scene2": {
        ...
        "step2": {
            "answer": True,
            "GPT_character": "鈴木あい",
            "GPT_talk": "まずはユーザーに対して「話しかけてくれてありがとう、私、鈴木あいです！」を言ってから、趣味について話す",
            "image":True,
            "imageclass":"ai",
            "next": "step9"
        },
        "step3": {
            "description": "隣に座っていた女子生徒が目を輝かせながら私に声をかけてきた。「こんにちは!私、佐々木美咲。このクラスで一緒になれてうれしいな。自己紹介、聞かせてもらったよ。」",
            "image":True,
            "imageclass":"misaki",
            "answer": True,
            "button": False,
        ...
        "step4": {
            "description": "明日まで出す課題が出され、下校のベルが鳴った。美咲が私に向かって、ニコニコしながら提案してきた。美咲:「ねえ、一緒に帰らない？」",
            "button": True,
            "options": ["次へ"],
            "next": "step5"
        },
        ...
        "step6": {
            "description": "会話が途中で中断され、先生が夏休みの宿題の提出を促してきた。宿題が多く、提出するのにも時間がかかった。あいがため息をつきながら言った:「疲れたー。夏休みの宿題、どうやって終わらせました？私は計画を立てて、毎日ちょっとずつ取り組んでいました。」",
            "answer": True,
            "next": "step10"
        },
        "step7": {
        ...
        "step9": {
            "answer": True,
            "GPT_character": "鈴木あい",
            "GPT_talk": "ユーザーに対して返事する",
            "next": "step6"
        },
        "step10": {
            "answer": True,
            "GPT_character": "佐々木美咲",
            "GPT_talk": "ユーザーに対して返事して、いい感じにこの話題を終わらせる",
            "next": "step4"
        },
        "step12": {
            "answer": True,
    ...
    "scene4": {
        "step1": {
        "description":"美咲と一緒にレストランに入ると、私たちは温かく迎え入れられ、席に案内された。テーブルに着くと、ウェイターがメニューを持ってきてくれた。メニューを開いてみると、さまざまな美味しそうな料理が並んでいて、選ぶのが難しいほどだった。",
        "button":True,
        "options": ["次へ"],
        "next": "step4"
        },
        "step2": {
        "description":"みんなでレストランに入ると、私たちは温かく迎え入れられ、席に案内された。テーブルに着くと、ウェイターがメニューを持ってきてくれた。メニューを開いてみると、さまざまな美味しそうな料理が並んでいて、選ぶのが難しいほどだった。",
        "button":True,
        "options": ["次へ"],
        "next": "step4"
        },
        "step3": {
        "description":"レストランに入ると、私たちは温かく迎え入れられ、席に案内された。テーブルに着くと、ウェイターがメニューを持ってきてくれた。メニューを開いてみると、さまざまな美味しそうな料理が並んでいて、選ぶのが難しいほどだった。",
        "button":True,
        "options": ["次へ"],
        "next": "step4"
        },
        "step4": {
	        "description":"メニューは以下となります：好きな一品をお選びください。1.特製ハンバーグステーキ - 自家製ソースで仕上げたジューシーなハンバーグ。付け合わせには、季節の野菜とマッシュポテトが添えられています。2.鶏のクリーム煮 - 柔らかく煮込んだ鶏肉を、濃厚なクリームソースで味わう一品。バゲットが付いてきます。3.海鮮パエリア - 新鮮な海の幸がふんだんに使われた、彩り豊かなスペイン料理。サフランの香りが食欲をそそります。4.	野菜たっぷりグリーンカレー - 辛さ控えめのココナッツベースのカレー。たくさんの種類の野菜が入っていて、ヘルシーです。5.マルゲリータピザ - シンプルながらも、トマトの酸味とモッツァレラチーズの濃厚さが絶妙なバランスのピザ。",
	        "timecount":True,
	        "button":True,
	        "image":True,
	        "options": {
        ...
        "step5": {
	        "description":"これにした理由を教えていただけますか？(見た目がおしゃれとか)",
	        "answer":True,
					"imageclass":"hamburgersteak",
	        "next": {"END"}
        },
        "step6": {
	        "description":"これにした理由を教えていただけますか？(見た目がおしゃれとか)",
	        "answer":True,
					"imageclass":"creamchicken",
	        "next": {"END"}
        },
        "step7": {
	        "description":"これにした理由を教えていただけますか？(見た目がおしゃれとか)",
